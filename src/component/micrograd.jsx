...
class Value {
  ...
  add(other, l = '') {
    other = other instanceof Value ? other : new Value(other, [], '', l);
  ...
  mul(other) {
    ...
    const out = new Value(this.data * other.data, [this, other], '*');
    //for labeling the other to be a loss if it is part of the loss function
    //for the backpropgation graph
    // if (this.label === "loss") {
    //   other.setLabel('loss')
    // }
    out._backward = () => {
...
class Neuron extends Module {
  ...
  constructor(nin, nonlin = true) {
    ...
    this.w = Array(nin).fill().map(() => {
      const n = new Value(uniform(-1, 1) * scale)
      n.setLabel('weight')
      return n
    }
    );
    this.b = new Value(0);
    this.b.setLabel('bias')
    this.nonlin = nonlin;
  }
...
export function crossEntropy(logits, target) {
  ...
  const probs = ex.map(x => {
    // x.setLabel('loss')
    let dived = x.div(denom)
    dived.setLabel('loss')
    return dived
  }
